<script lang="ts">
	...
	// ---- HUD panel (host tools live in a labeled bottom dock — always on
	// screen for hosts, so the tools can't be missed the way a corner FAB
	// was) ----
	let hudOpen = $state(false);
	let matchesOpen = $state(false);
	let activeTool = $state<"match" | "courts" | "invite" | "club">("match");

	type HudTool = "match" | "courts" | "invite" | "club";

	const DOCK_SEEN_KEY = "deuce:hostdock-opened";
	let dockSeen = $state(
		typeof window !== "undefined" &&
			window.localStorage.getItem(DOCK_SEEN_KEY) === "1",
	);

	function openHud(tool: HudTool = activeTool) {
		activeTool = tool;
		hudOpen = true;
		if (!dockSeen) {
			dockSeen = true;
			try {
				window.localStorage.setItem(DOCK_SEEN_KEY, "1");
			} catch {
				// storage unavailable — the pulse just shows again next visit
			}
		}
	}

	function toggleHud(tool: HudTool) {
		if (hudOpen && activeTool === tool) {
			hudOpen = false;
			return;
		}
		openHud(tool);
	...
	// Swipe-up-from-the-bottom-edge, in addition to tapping the dock — an
	...
	// ---- relogin ----
	// Sessions end and their local tokens go stale, so getting back in never
	// restores stored *session* state — it routes through the club code
	// instead (/join's club-code tab recognizes returning devices and seats
	// them straight into whatever session is live now). A device that
	// visited before may still have the club's permanent join code cached
	// (via cacheClubInfo), which is enough to prefill the code field;
	// otherwise the plain /join page asks for it.
	let rejoinCode = $derived(
		(() => {
...
<PullToRefresh onrefresh={poll}>
	{#if !token}
		<section class="container rise-in">
			<div class="card">
				<p class="muted">
					{rejoinCode
						? "You're signed out on this device. Enter your club code to get back in — you'll be seated in the live session, with your rating and history intact."
						: "You'll need to join this session with your club code."}
				</p>
				<a
					href={rejoinLink}
				...
				>
					{rejoinCode ? "Sign back in" : "Join this session"}
				</a>
			</div>
		</section>
	{:else if loading}
	...
	{:else}
		<section class="container-wide rise-in" class:host-clearance={isHost}>
		...
		{#if isHost}
			<!-- Labeled dock instead of an unlabeled corner dot: every tool is
			     one tap away and impossible to miss. While the panel is open on
			     wide screens it doubles as the tab switcher. On phones the
			     bottom sheet covers this area, so it hides while open. -->
			<nav
				class="host-dock"
				class:attention={!dockSeen && !hudOpen}
				class:dock-raised={hudOpen}
				class:dock-hidden-mobile={hudOpen}
				aria-label="Host tools"
			>
				<span class="dock-title">Host</span>
				<button
					type="button"
					class="dock-btn"
				...
				>
					...
					Match
					{#if waiting.length}
						<span class="dock-count">{waiting.length}</span>
					{/if}
				</button>
				<button
					type="button"
		...
		{#if hudOpen}
			...
			<div class="hud-panel rise-in">
				<div class="hud-panel-head">
					<div class="spread">
						<strong class="panel-title">Host tools</strong>
						<button
							type="button"
							class="link-btn panel-close"
							onclick={() => (hudOpen = false)}
						...
						{#if session.status !== "FINISHED"}
							...
							<div class="card builder">
								...
								{:else if session.assignment_mode === "AUTOMATIC" && session.auto_fill_enabled}
									...
									<p class="muted small">
										Auto-fill is on — open courts fill
										themselves as soon as 4 players are
										waiting. Turn it off to assign manually.
									</p>
								{:else if session.assignment_mode === "AUTOMATIC"}
									<div class="stack">
										...
										<p class="faint small">
											The engine picks the fairest 4 from
											the queue; you review teams and pick
											a court before it starts.
										</p>
									</div>
								{:else}
									<div class="stack">
...
<style>
	...
	/* The dock is fixed over the viewport bottom — reserve space under the
	   content so the last section can scroll fully clear of it. */
	.host-clearance {
	...
	/* first-run pulse so a new host notices the controls */
	.host-dock.attention {
		animation: dock-glow 1.7s ease-in-out infinite;
	}
	...
	/* While the panel is open: raise above the backdrop on wide screens so
	   the dock doubles as the tab switcher (parked bottom-left, out of the
	   side panel's way), and hide on phones where the sheet covers it. */
	.host-dock.dock-raised {
		z-index: 72;
	}

	@media (max-width: 719.98px) {
		.host-dock.dock-hidden-mobile {
			display: none;
		}
	}
	...
	/* Narrow phones: drop the HOST chip and tighten padding so the four
	   labeled tabs always fit without clipping. */
	...
	/* live match card: accent edge + traveling shimmer so "playing" reads
	   as motion even from across the gym */
	.match-card-live {
		position: relative;
		overflow: hidden;
	...
	.match-card-live::before {
		...
		animation: court-sweep 2.8s ease-in-out infinite;
		pointer-events: none;
	}
